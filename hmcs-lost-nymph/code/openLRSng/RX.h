...
Serial_ *mainSerial = &Serial;
Serial_ *hwSerial   = &Serial;

#else
// TODO: EEPROM for Teensy LC

Serial_ *mainSerial = &Serial;
Serial_ *hwSerial   = &Serial1;
...
void setup(void)
{
  ...
  mainSerial->print("OpenLRSng RX starting ");
  printVersion(version);
  mainSerial->println(" on HW " XSTR(BOARD_TYPE));
  ...
  if (checkIfConnected(OUTPUT_PIN[0], OUTPUT_PIN[1]) || (!bindReadEeprom())) {
    mainSerial->print("EEPROM data not valid or bind jumpper set, forcing bind\n");
    if (bindReceive(0)) {
      bindWriteEeprom();
      mainSerial->println("Saved bind data to EEPROM\n");
      Green_LED_ON;
    }
    setupOutputs();
  } else {
  ...
  mainSerial->print("Entering normal mode");
  ...
  if ((rx_config.pinMapping[TXD_OUTPUT] == PINMAP_SPKTRM) ||
    ...
    hwSerial->begin(100000, SERIAL_8E2); // Even parity, two stop bits
    ...
    if (bind_data.serial_baudrate < 10) {
      ...
      hwSerial->begin(bind_data.serial_baudrate);
    }
  }
  ...
  #if (BOARD_TYPE != 10) // HMCS Lost Nymph
  // TODO: HMCS Lost Nymph
  #endif // HMCS Lost Nymph
}
...
uint8_t bindReceive(uint32_t timeout)
{
  ...
  init_rfm(1);
  rx_reset();
  mainSerial->println("Waiting bind\n");
  while ((!timeout) || ((millis() - start) < timeout)) {
    if (RF_Mode == RECEIVED) {
      mainSerial->println("Got pkt");
...
void checkSerial(void)
{
  while (hwSerial->available() && (((serial_tail + 1) % SERIAL_BUFSIZE) != serial_head)) {
    serial_buffer[serial_tail] = hwSerial->read();
...
void checkBinaryMode(void)
{
  if ((hwSerial->available() > 3) &&
      (hwSerial->read() == 'B') && (hwSerial->read() == 'N') &&
      (hwSerial->read() == 'D') && (hwSerial->read() == '!')) {
    delay(250);
    while(hwSerial->available()) {
      if(hwSerial->read() == 'B') {
        binaryMode();
