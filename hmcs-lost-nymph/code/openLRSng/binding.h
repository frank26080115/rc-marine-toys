...
{
  uint8_t retries = 5;
  while ((--retries) && (data != EEPROM.read((uint8_t *)addr))) {
    EEPROM.write((uint8_t *)addr, data);
  }
  if (!retries) {
    fatalBlink(2);
  }
...
start:
    ...
    for (uint8_t i = 0; i < dataSize; i++, addressNeedle++) {
      if (!write) {
        *((uint8_t*)dataAddress + i) = EEPROM.read(addressNeedle);
      } else {
    ...
    if (!write) {
      CRC = (EEPROM.read(addressNeedle) << 8) | EEPROM.read(addressNeedle + 1);
