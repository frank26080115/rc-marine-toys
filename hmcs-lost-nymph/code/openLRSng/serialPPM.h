...
#if (BOARD_TYPE != 10)
HardwareSerial *ppmSerial = &Serial;
#else
HardwareSerial *ppmSerial = &Serial1;
#endif
...
void sendSBUSFrame(uint8_t failsafe, uint8_t lostpack)
{
  ...
  if ((now - sOutLast) > 10000) {
    ...
    ppmSerial->write(SBUS_SYNC);
    for (uint8_t i = 0; i<23; i++) {
      ppmSerial->write(sbus.bytes[i]);
    }
    ppmSerial->write(SBUS_TAIL);
  }
}
...
void sendSpektrumFrame(void)
{
  ...
  if ((now - sOutLast) > 10000) {
    ...
    sOutLast = now;
    ppmSerial->write(SPKTRM_SYNC1);
    ppmSerial->write(SPKTRM_SYNC2);
    for (uint8_t i = 0; i < 7; i++) {
      ppmSerial->write((ch << 2) | ((PPM[ch] >> 8) & 0x03));
      ppmSerial->write(PPM[ch] & 0xff);
      ch++;
    }
  }
}
...
void sendSUMDFrame(uint8_t failsafe)
{
  ...
  if ((now - sOutLast) > 10000) {
    ...
    ppmSerial->write(CRC16_value >> 8);
    ppmSerial->write(CRC16_value & 0xff);
  }
}
